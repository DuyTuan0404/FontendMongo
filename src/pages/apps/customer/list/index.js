...
const CustomerList = ({ apiData }) => {
      ...
      <>
        ...
        >
          <MenuItem
            component={Link}
            sx={{ '& svg': { mr: 2 } }}
            href='/apps/customer/view/account'
  ...
  const columns = [
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <CustomAvatar
              ...
              color={customerRoleObj[row.role].color || 'primary'}
            >
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <CustomAvatar
              ...
              color={customerRoleObj[row.role].color || 'primary'}
            >
              <Icon icon={customerRoleObj[row.role].icon} />
            </CustomAvatar>
            <Typography noWrap sx={{ color: 'text.secondary', textTransform: 'capitalize' }}>
              {row.role_name}
            </Typography>
    ...
    },
    {
      ...
      field: 'actions',
      headerName: 'Actions',
      renderCell: ({ row }) => <RowOptions id={row.id} row={row} onEdit={handleEditCustomer} />
    }
  ]
    ...
    <Grid container spacing={6.5}>
      ...
      <Grid item xs={12}>
        <Card>
          ...
          <TableHeader value={value} handleFilter={handleFilter} toggle={handleAddCustomer} />
          <DataGrid
            autoHeight
            rowHeight={62}
            rows={store.data}
      ...
      <AddCustomerDrawer
        ...
        onSubmit={(data) => {
          // Xử lý submit edit ở đây nếu cần, ví dụ dispatch(updateCustomer(data))
