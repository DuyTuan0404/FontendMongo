...
const CustomerDrawer = props => {
  // ** Props
  const { open, toggle, customer, mode = 'add', onSubmit: onSubmitProp } = props
  // ** State
  const [plan, setPlan] = useState(customer?.currentPlan || 'basic')
  const [role, setRole] = useState(customer?.role || 'subscriber')
  ...
  // Nếu là edit, lấy defaultValues từ customer, nếu không thì dùng defaultValues mặc định
  const isEdit = mode === 'edit'
  // Track previous values to detect customer-initiated changes
  ...
  useEffect(() => {
    if (cityValue) {
      ...
      const cityId = cityObj?.id || cityValue
      dispatch(fetchDistricts(cityId))

      // Chỉ reset district/ward nếu đây là thay đổi từ người dùng, không phải lần tải đầu tiên
      if (!isInitialLoad.current) {
        setValue('district', '')
        setValue('ward', '')
      }
    }
  }, [cityValue, dispatch, setValue, cities?.data])
  ...
  useEffect(() => {
    if (districtValue) {
      ...
      const districtId = districtObj?.id || districtValue
      dispatch(fetchWards(districtId))
      if (!isInitialLoad.current) {
        setValue('ward', '')
      }
    }
  }, [districtValue, dispatch, setValue, districts?.data])

  // Reset form khi customer hoặc open thay đổi (edit)
  useEffect(() => {
      // Helper function to safely extract ID from populated fields
    const getSafeId = (value) => {
      if (typeof value === 'string') return value; // Already an ID
      if (Array.isArray(value) && value.length > 0) {
        return value[0]?._id || value[0]?.id || null; // Handle array of objects
      }
      if (typeof value === 'object' && value !== null) {
        return value._id || value.id || null; // Handle single object
      }
      return ''; // Fallback to empty string for controlled components
    };

    if (isEdit && customer) {
      isInitialLoad.current = true; // Đánh dấu là lần tải đầu tiên
      reset({
        ...defaultValues,
        ...customer,
        city: getSafeId(customer.city),
        district: getSafeId(customer.district),
        ward: getSafeId(customer.ward),
        password: '',
        passwordConfirm: ''
      })
      setPlan(customer.currentPlan || 'basic')
      setRole(customer.role || 'subscriber')

      // Initialize avatar type/value/preview from customer
      const initialAvatarType = avatarTypeReverseMap?.[customer.avatar_type] || 'upload'
      ...
      if (initialAvatarType === 'imageLink' || initialAvatarType === 'videoLink') {
        ...
        if (typeof customer.avatar === 'string') setValue('avatar', customer.avatar)
        setAvatarPreview(initialAvatarType === 'imageLink' && typeof customer.avatar === 'string' ? customer.avatar : null)
        setAvatarError('')
      } else {
        // For upload type, we cannot prefill a File; show preview if we have a URL/path
        setValue('avatar', '')
        if (typeof customer.avatar === 'string' && customer.avatar) {
  ...
  }, [customer, open, isEdit, reset, setValue])
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 6 }}>
        <form id='add-customer-form' onSubmit={handleSubmit(handleFormSubmit)}>
          <Controller
            name='name'
            control={control}
            rules={{ required: true }}
          ...
          />
            ...
            <Controller
              name='avatarColor'
              control={control}
              render={({ field: { value = 'success', onChange } }) => (
                <CustomTextField
                  select
                  fullWidth
                  sx={{ mb: 4 }}
                ...
                >
                  <MenuItem value=''>Select Avatar Color</MenuItem>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Button type='submit' variant='contained' sx={{ mr: 3 }} form='add-customer-form'>
            Submit
          </Button>
